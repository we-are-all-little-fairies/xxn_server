...
async function findUser(mapping) {
    return new Promise((resolve, reject) => {
        MongoDBClient.collection("user").findOne(mapping).then(res => {
...
async function decreaseTryTime(mapping) {
    return new Promise((resolve, reject) => {
        MongoDBClient.collection("user").findOneAndUpdate(mapping, {
            $inc: {
                retry: -1
            }
        }).then(res => {
...
module.exports.insertUser = async function insertUser(name, password, token) {
    ...
    if (await findUser({name: name}) != null) {
        ...
        return MongoDBClient.collection("user").insertOne({name: name, password: password, token: token, retry: 10})
    }
